...
Le site vend un produit (des places de match), pas seulement une vitrine du club. La base de données `ui-ux-pro-max` (`data/colors.csv`) confirme que la palette bleu marine foncé déjà en place correspond exactement au profil « Ticketing / Box Office » (`#0F172A` primaire), et recommande d'y ajouter **un seul accent à fort contraste, réservé à l'action d'achat** (« available green ») plutôt que de multiplier les couleurs.

## Couche 1 — Primitives (valeurs brutes)

| Token | Valeur | Source |
|---|---|---|
| `cta-600` | `#16a34a` | `ui-ux-pro-max` → profil Ticketing/Box Office, accent « available » |
| `cta-500` / `cta-400` | `#22c55e` / `#4ade80` | Nuances de la même teinte (hover / fond léger) |
| `cta-700` | `#15803d` | Nuance foncée (texte sur fond clair, focus ring) |
| `soldout-600` | `#dc2626` | `ui-ux-pro-max` → même profil, accent « sold-out » |

    ...
    <>
      ...
      <section className="bg-navy-950 py-16 text-white">
        <div className="container-page">
          ...
          {isError && <ErrorState />}
          {homeMatches[0] && (
            <div className="mt-8 inline-flex flex-col gap-3 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
      ...
      <Section eyebrow="Tarifs" title="Choisissez votre place">
        <div className="grid gap-6 sm:grid-cols-3">
          {TIERS.map((tier) => (
            <div key={tier.name} className="rounded-3xl border border-navy-100 bg-white p-6 shadow-sm transition-shadow hover:shadow-xl">
              <p className="text-sm font-medium uppercase tracking-wide text-cta-600">{tier.name}</p>
              <p className="mt-2 font-display text-3xl font-semibold text-navy-950">{tier.price}</p>
      ...
      {homeMatches.length > 0 && (
        <Section eyebrow="Calendrier" title="Tous les matchs à domicile" tone="dark">
          <div className="grid gap-4 sm:grid-cols-2">
            {homeMatches.map((m) => (
              <div key={m.id} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-5">
                <div>
                  <p className="font-display font-medium text-white">{m.home} vs {m.away}</p>
